...
const PlaceOrder = () => {
  ...
  const { navigate, BACKEND_URL, token, cartItem, fetchCartItems, getCartTotalAmount, products } = useShopContext()
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      let orderData = {
        address: formData,
        items: orderItems,
        amount: getCartTotalAmount() + 40
      }
      switch (method) {
        case 'cod':
            const response = await axios.post(`${BACKEND_URL}/user/placeorder`,
          ...
          <div className='mt-12'>
            ...
            <div className="flex gap-3 flex-col lg:flex-row">
              <div onClick={() => setMethod('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
                ...
                <img src={stripe_logo} className='h-5 mx-4' alt="" />
              </div>
